...
type WebSearchConfig = Record<string, unknown> | undefined;
...
function resolveSearchConfig(): WebSearchConfig {
  return undefined;
}
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message:
      "web_search needs a Brave Search API key. Set BRAVE_API_KEY in the environment.",
  };
}
...
async function runGrokSearch(params: {
  ...
  if (!res.ok) {
    ...
    const detail = detailResult.text;
    throw new Error(
      `xAI API error (${res.status}): ${detail || res.statusText}`,
    );
  }
